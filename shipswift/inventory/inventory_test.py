...
def test_update_product(client):
    #Succesful update 
    rv = client.post("/api/inventory/add", json={'name': 'test_product', 'category': 'test_category', 'price': 10.99, 'description': 'Test product description', 'count': 50})
    rv1 = client.put("/api/inventory/update/test_product", json={'name': 'test_product', 'category': 'test_category_updated', 'price': 10.99, 'description': 'Test product description', 'count': 50})
    assert rv1.status_code == 200  # Assuming a successful response status code
    ...
    rv3 = client.post("/api/inventory/add", json={'name': 'test_product1', 'category': 'test_category', 'price': 10.99, 'description': 'Test product description', 'count': 50})
    rv4 = client.put("/api/inventory/update/test_product1", json={'name': 'test_product', 'category': 'test_category', 'price': 10.99, 'description': 'Test product description', 'count': 50})
...
def test_deduce_product(client):
    ...
    assert rv.status_code == 200  # Assuming a successful response status code
    assert rv.get_json() == {"message": "Item test_product1 Does Not Exist"}
